...
from collective.opensearch.tests import BaseFunctionalTest
...
class TestExtractRestApiEndpoint(BaseRedisTest):
    def setUp(self):
        ...
        self.endpoint = f"{self.portal_url}/@opensearch_extractdata"
    ...
    def test_extract_all_data_via_endpoint(self):
        ...
        self.assertDictEqual(
            json.loads(json.dumps(processor.get_data_for_os(self.obj.UID()))), content
        )
...
class TestIndexBlobs(BaseRedisTest):
    ...
    def test_index_data_from_file(self):
        ...
        os_results = self.catalog(**query)
        self.assertEqual(1, len(os_results), "Expect 1 item")
    ...
    def test_update_and_delete_file(self):
        ...
        query = {"SearchableText": "lorem"}
        os_results = self.catalog(**query)
        self.assertEqual(0, len(os_results), "Expect no item")

    def test_make_sure_binary_data_are_removed_from_os(self):
        file_ = self._setup_sample_file()
        os_data = self.os.connection.get(self.os.index_name, file_.UID())
        self.assertIsNone(os_data["_source"]["attachments"][0]["data"])

    def test_multiple_file_fields(self):
        fti = self.portal.portal_types.File
        self._set_model_file(fti, "collective.opensearch.tests:test_file_schema.xml")
        ...
        os_data = self.os.connection.get(self.os.index_name, file_.UID())
        self.assertIsNone(os_data["_source"]["attachments"][0]["data"])
        self.assertIsNone(os_data["_source"]["attachments"][1]["data"])
        ...
        query = {"SearchableText": "lorem"}
        os_results = self.catalog(**query)
        self.assertEqual(0, len(os_results), "Expect 0 item")
    ...
    def test_dont_queue_blob_extraction_jobs_if_not_possible(self):
        ...
        self.os.connection.indices.put_settings(body=settings, index=self.os.index_name)
        self.os.connection.ingest.delete_pipeline("cbor-attachments")
        file_path = os.path.join(os.path.dirname(__file__), "assets/test2.docx")
        with io.FileIO(file_path, "rb") as pdf:
    ...
    def test_do_not_index_data_from_images(self):
        ...
        os_data = self.os.connection.get(self.os.index_name, _image.UID())
        self.assertNotIn(
            "attachments",
            os_data["_source"],
