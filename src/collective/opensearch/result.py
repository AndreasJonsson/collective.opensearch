...
from collective.opensearch.utils import get_brain_from_path
...
class OpenSearchBrain:
    """A Brain containing only information indexed in OpenSearch."""
    ...
    def __getattr__(self, name):
        if not self.__contains__(name):
            raise AttributeError(
                f"'OpenSearchBrain' object has no attribute '{name}'"
            )
...
def BrainFactory(manager):
    def factory(result: dict) -> Union[AbstractCatalogBrain, OpenSearchBrain]:
        ...
        if path:
            ...
            if not brain:
                ...
                brain = OpenSearchBrain(record=result, catalog=catalog)
