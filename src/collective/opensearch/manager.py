from collective.opensearch import interfaces
from collective.opensearch import local
from collective.opensearch import logger
from collective.opensearch import utils
from collective.opensearch.result import BrainFactory
from collective.opensearch.result import OpenSearchResult
from collective.opensearch.utils import use_redis
...
CONVERTED_ATTR = "_opensearchconverted"
CUSTOM_INDEX_NAME_ATTR = "_opensearchcustomindex"
INDEX_VERSION_ATTR = "_opensearchindexversion"


@implementer(interfaces.IOpenSearchManager)
class OpenSearchManager:

    _catalog: CatalogTool = None
    connection_key = "opensearch_connection"
    ...
    def raise_search_exception(self):
        ...
        try:
            value = api.portal.get_registry_record(
                "raise_search_exception", interfaces.IOpenSearchSettings, False
            )
        except KeyError:
            value = False
        return value
    ...
    def bulk_size(self) -> int:
        """Bulk size of OpenSearch calls."""
        try:
            value = api.portal.get_registry_record(
                "bulk_size", interfaces.IOpenSearchSettings, 50
            )
        except KeyError:
            value = 50
        return value
    ...
    def highlight(self):
        ...
        try:
            value = api.portal.get_registry_record(
                "highlight", interfaces.IOpenSearchSettings, False
            )
        except KeyError:
            value = False
        return value
    ...
    def highlight_threshold(self):
        ...
        try:
            value = api.portal.get_registry_record(
                "highlight_threshold", interfaces.IOpenSearchSettings, False
            )
        except KeyError:
            value = 600
        return value
    ...
    def highlight_pre_tags(self):
        ...
        try:
            value = api.portal.get_registry_record(
                "highlight_pre_tags", interfaces.IOpenSearchSettings, ""
            )
        except KeyError:
            value = ""
        if value is None:
    ...
    def highlight_post_tags(self):
        ...
        try:
            value = api.portal.get_registry_record(
                "highlight_post_tags", interfaces.IOpenSearchSettings, ""
            )
        except KeyError:
            value = ""
        if value is None:
    ...
    def enabled(self):
        try:
            value = api.portal.get_registry_record(
                "enabled", interfaces.IOpenSearchSettings, False
            )
        except KeyError:
            value = False
        return value
    ...
    @property
    def info(self) -> list:
        """Return Information about OpenSearch."""
        ...
        try:
            ...
            try:
                ...
                return [
                    ...
                    ("OpenSearch Version", cluster_version),
                ...
                return [
                    ("Cluster Name", cluster_name),
                    ("OpenSearch Search Version", cluster_version),
                    ("Number of docs (Catalog)", catalog_docs),
                ]
    ...
    def _recreate_catalog(self):
        ...
        if self.index_version:
            try:
                ...
                pass
        self.flush_indices()
        self._convert_catalog_to_opensearch()

    def _convert_catalog_to_opensearch(self):
    ...
    def _setup_attachment_pipeline_and_default_index(self):
        ...
        2. Iterate over all extracted data and extend the SearchableText.
           ...
           Version 8 of elastisearch we could use the remove_binary flag, but
    ...
    @property
    def connection(self) -> OpenSearch:
        conn = local.get_local(self.connection_key)
        if not conn:
            hosts, params = utils.get_connection_settings()
            local.set_local(self.connection_key, OpenSearch(hosts, **params))
            conn = local.get_local(self.connection_key)
        return conn
    ...
    def _bulk_call_redis(self, batch):
        from collective.opensearch.redis.tasks import bulk_update
    ...
    def update_blob(self, item):
        from collective.opensearch.redis.tasks import update_file_data
    ...
    def search(self, query: dict, factory=None, **query_params) -> LazyMap:
        ...
        @param factory: The factory that maps each opensearch result.
        ...
        result = OpenSearchResult(self, query, **query_params)
    ...
    def search_results(self, request=None, check_perms=False, **kw):
        ...
        processQueue()
        if not (self.active and utils.getOSOnlyIndexes().intersection(kw.keys())):
            method = (
