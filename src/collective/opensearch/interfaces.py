...
class IOpenSearchLayer(Interface):
    pass


class IOpenSearchManager(Interface):
    pass
...
class IOpenSearchSettings(Interface):
    ...
    os_only_indexes = schema.Set(
        title="Indexes for which all searches are done through OpenSearch",
    ...
    timeout = schema.Float(
        title="Read timeout",
        description="how long before timeout connecting to opensearch",
        default=2.0,
    )

    bulk_size = schema.Int(
        title="Bulk Size", description="bulk size for opensearch queries", default=50
    )

    highlight = schema.Bool(
        title="Enable Search Highlight",
        description="Use opensearch highlight feature instead of descriptions in search results",
        default=False,
        required=False,
    )
    ...
    raise_search_exception = schema.Bool(
        ...
        description="If there is an error with opensearch Plone will default to trying the old catalog search. Set this to true to raise the error instead.",
        default=False,
        required=False,
    )


class IOpenSearchIndexQueueProcessor(IIndexQueueProcessor):
    """Index queue processor for opensearch."""
