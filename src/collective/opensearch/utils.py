...
def get_settings():
    """Return IOpenSearchSettings values."""
    registry = getUtility(IRegistry)
    try:
        settings = registry.forInterface(IOpenSearchSettings, check=False)
...
def get_connection_settings():
    ...
    return settings.hosts, params


def getOSOnlyIndexes():
    settings = get_settings()
    try:
        indexes = settings.os_only_indexes
