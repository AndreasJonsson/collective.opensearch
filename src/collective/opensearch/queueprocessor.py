...
from collective.opensearch.interfaces import IAdditionalIndexDataProvider
from collective.opensearch.interfaces import IOpenSearchIndexQueueProcessor
from collective.opensearch.interfaces import IndexingActions
from collective.opensearch.interfaces import IReindexActive
...
@implementer(IOpenSearchIndexQueueProcessor)
class IndexProcessor:
    """A queue processor for opensearch"""

    _manager: OpenSearchManager = None
    _os_attributes = None
    ...
    def manager(self):
        ...
        if not self._manager:
            self._manager = OpenSearchManager()
    ...
    @property
    def os_attributes(self):
        """Return all attributes defined in portal catalog."""
        if not self._os_attributes:
            self._os_attributes = getOSOnlyIndexes()
        return self._os_attributes
    ...
    def all_attributes(self):
        ...
        if not self._all_attributes:
            ...
            os_indexes = self.os_attributes
            catalog_indexes = set(catalog.indexes())
            self._all_attributes = os_indexes.union(catalog_indexes)
    ...
    def get_data_for_os(self, uuid, attributes=None):
        """Data to be sent to opensearch."""
        ...
        for index_name in attributes:
            ...
            if index is not None:
                ...
                if value in (None, "None"):
                    ...
                    value = None
            elif index_name in self.os_attributes:
